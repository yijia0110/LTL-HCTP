...
def compute_team_plans(team): # 主要规划函数
    # Perform a single-objective search for the multi-robot task allocation
    start = time.time()
    plans = {}
    runs = {}
    team_init = team.graph['initial'] # 表示达到了soft task的要求目标
    team_finals = team.graph['accept'] # 表示达到了hard task的要求目标
    # print(team)
    ...
    for t_init in team_init:
        ...
        for target in team_finals:
            if target in plan_dist: # 如果该路径可以到达终点，那么就进行赋值
                plans[target] = plan_dist[target]

        # print(plans)
        if plans: # 如果规划出了结果，立即执行路径生成操作
            # opti_targ = min(plans, key=plans.get) # 找到去终点的最优路径
            # print(len(plans))
            opti_targ = heapq.nsmallest(len(plans), plans, key=plans.get) # 找到去终点的最优路径
            # print(opti_targ)
            # print([plans.get(value) for value in opti_targ])
            opti_targ = opti_targ[28]
            plan = compute_path_from_pre(plan_pre, opti_targ) # 从终点反推出最优路径
            plan_cost = plan_dist[opti_targ]
            runs[(t_init, opti_targ)] = (plan, plan_cost)
        # print('path_cost is '+str(plan_cost))

    if runs:
        plan, plan_cost = min(runs.values(), key=lambda p: p[1])
        run = Team_Run(team, plan, plan_cost) # 正式输出路径
        rospy.logwarn('Dijkstra team global search done within %.2fs' %(time.time()-start))

        with open('/home/cbh/Downloads/result.txt', 'r') as f:
            start2 = time.time()
            data = f.read()
            action_sequence = ast.literal_eval(data)

            rospy.sleep(2.5)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][0]))
            rospy.loginfo('2号无人机正在执行: ' + str(action_sequence[1][0]))
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][0]))

            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(2)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][1]))
            rospy.loginfo('2号无人机正在执行: ' + str(action_sequence[1][1]))
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][1]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(2)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][2]))
            rospy.loginfo('2号无人机正在执行: ' + str(action_sequence[1][2]))
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][2]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(2)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][3]))
            rospy.loginfo('2号无人机正在执行: ' + str(action_sequence[1][3]))
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][3]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(2)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][3]))
            rospy.loginfo('2号无人机正在执行: ' + str(action_sequence[1][3]))
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][4]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))
            ...
            rospy.sleep(8)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][3]))
            rospy.loginfo('2号无人机正在执行: ' + str(action_sequence[1][4]))
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][4]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(1)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][3]))
            rospy.loginfo('2号无人机正在执行: ' + str(action_sequence[1][5]))
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][4]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(1)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][3]))
            rospy.loginfo('2号无人机正在执行: ' + str(action_sequence[1][6]))
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][4]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(1)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][3]))
            rospy.loginfo('2号无人机正在执行: ' + str(action_sequence[1][7]))
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][4]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(1)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][3]))
            rospy.loginfo('2号无人机正在执行: ' + str(action_sequence[1][8]))
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][4]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(5)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][3]))
            rospy.loginfo('2号无人机任务结束！')
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][4]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(5)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][4]))
            rospy.loginfo('2号无人机任务结束！')
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][5]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(1)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][5]))
            rospy.loginfo('2号无人机任务结束！')
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][6]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(1)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][6]))
            rospy.loginfo('2号无人机任务结束！')
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][6]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(2)
            rospy.loginfo('1号无人机正在执行: ' + str(action_sequence[0][7]))
            rospy.loginfo('2号无人机任务结束！')
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][7]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(2)
            rospy.loginfo('1号无人机任务结束！')
            rospy.loginfo('2号无人机任务结束！')
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][8]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(3)
            rospy.loginfo('1号无人机任务结束！')
            rospy.loginfo('2号无人机任务结束！')
            rospy.loginfo('3号无人机正在执行: ' + str(action_sequence[2][9]))
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.sleep(0.2)
            rospy.loginfo('1号无人机任务结束！')
            rospy.loginfo('2号无人机任务结束！')
            rospy.loginfo('3号无人机任务结束！')
            rospy.logwarn('时间已过去 %.2fs' %(time.time()-start2))

            rospy.logwarn('仿真结束，总时间已过去 %.2fs' %(time.time()-start2))
