...
class TSModel(DiGraph):
    ...
    def build_full(self):
        ...
        # rospy.loginfo("当前无人机状态空间共有状态 %d 个，转移 %s 个" %(len(self.nodes()), len(self.edges())))
        # rospy.loginfo("当前无人单位的初始状态为 %s" %str(self.graph['initial']))
        
        print("当前无人机状态空间共有状态 %d 个，转移 %s 个" %(len(self.nodes()), len(self.edges())))
        print("当前无人单位的初始状态为 %s" %str(self.graph['initial']))
