...
def compute_team_plans_multi_objective(team):
    ...
    if len(team_init) != 1 :
        ...
        print('team_init set variable not length of 1')
            ...
            for node in path[1:]:
                ...
                if team.projection(node)[0] == agent :
                    if team[prev_node][node]['action'] == 'switch_transition':
                        ...
                        print('switch transition detected out of sync')
    ...
    if best_max_agent_cost != float('inf'):
        ...
        print('Dijkstra team search done within %.2fs' %(time.time()-start))
...
def compute_team_plans(team): # 主要规划函数
    ...
    print('Dijkstra team global search start')
    ...
    if runs:
        ...
        print('Dijkstra team global search done within %.2fs' %(time.time()-start))
...
def compute_local_plan(team, rname):
    ...
    if runs:
        ...
        print('Dijkstra local PA search done within %.2fs' %(time.time()-start))
...
def find_reusable_plan(team, rname, old_run):
    ...
    for i in range(len(local_pa_plan)):
        if updated_init == local_pa_plan[i]:
            ...
            print('Reusable path found within %.2fs' %(time.time()-start))
            break
