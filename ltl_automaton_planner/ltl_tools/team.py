...
class TeamModel(DiGraph):
    def __init__(self, product_list, decomposition_set):
        DiGraph.__init__(self, pro_list=product_list, decomposition_set=decomposition_set, ts_initials=set(), initial=set(), accept=set()) # 这里用了很多自定义属性

    def build_team(self): # 在此处加入了无人机任务间的转换
        ...
        for idx, prod in enumerate(self.graph['pro_list']):
            for node in prod.nodes:
                ...
                team_node = (idx, ts_node, buchi_node)# 将数据进行组合
                # buchi doesn't have to be initial for all the agents，这句话说的是不需要为所有智能体都进行初始化，这是为什么呢？
                ts_initial = (idx, ts_node)
                if not self.has_node(team_node):
                    self.add_node(team_node, rname=idx, ts=ts_node, buchi=buchi_node, label=ts_node) # rname是当前任务的所属的无人机标签
        ...
        print('Team construction took %.2fs (can be avoided for global reallocation)' %(time.time()-start))

        self.add_switch_transition() # 加入转换符
        ...
        print('全体buchi自动机包含 %d 个可分解状态' %(len(self.graph['decomposition_set'])))
        print('全体无人集群共有状态 %d 个，转移 %s 个' %(len(self.nodes()), len(self.edges())))


    def add_switch_transition(self): # 这个状态转移函数的缺点是无法跨状态进行转移，且无法根据当前路径的损失值进行智能转移
        # Add switch transition between different robots
        for node in self.nodes:
            # print(self.nodes[node])
            if self.nodes[node]['rname'] == len(self.graph['pro_list'])-1: # 如果已经分配到了最后一个任务，则不需要分配转换符了，直接跳过
                continue

            if self.nodes[node]['buchi'] in self.graph['decomposition_set']: # 如果当前节点可以在分解后的集合中找到，则进行转换
                next_rname = self.nodes[node]['rname'] + 1 # 进行转换
                next_ts_state = None # 先给状态换新
                for next_team_init in self.graph['ts_initials']: # 因为需要转换，所以需要找到各无人单元的初始状态
                    if next_team_init[0] == next_rname: # 找到转换之后的无人单元TS状态
                        next_ts_state = next_team_init[1] # 将转换后的TS状态提取出来

                if next_ts_state == None: # 如果TS状态提取不成功，那么就报错
                    raise AssertionError()

                next_buchi_state = self.nodes[node]['buchi'] # 提取buchi自动机的状态
                next_node = (next_rname, next_ts_state, next_buchi_state) # 进行组合
    ...
    def revise_team(self, trace_dic, rname, old_run):
        ...
        for id in range(len(trace_dic)):
            ...
            try:
                ...
                print('Failed to set initial ts')
...
class Team_Run(object): # 用于输出规划的结果
    ...
    def plan_output(self, team):
        ...
        #     data = f.read()
        #     self.action_sequence = ast.literal_eval(data)

        for r_idx, act_seq in self.action_sequence.items():
            if len(act_seq) == 0:
                # rospy.loginfo('LTL Planner: Robot-%d plan: Empty; no task assigned' %(r_idx+1))
                print('LTL Planner: Robot-%d plan: Empty; no task assigned' %(r_idx+1))
            else:
                # rospy.loginfo('LTL Planner: Robot-%d plan: ' %(r_idx+1) + str(act_seq))
                # rospy.loginfo('%d 号无人机的任务序列为: ' %(r_idx+1) + str(act_seq))
                print('%d 号无人机的任务序列为: ' %(r_idx+1) + str(act_seq))
        # with open('/home/cbh/Downloads/result.txt', 'w') as f:
        #     print(self.action_sequence, file=f)

            # print('Total_cost for Robot-%d is: ' %r_idx + str(self.totalcost))
