    ...
    def build_full(self):
        ...
        for f_ts_node in self.graph['ts'].nodes():
            for f_buchi_node in self.graph['buchi'].nodes():
                ...
                for t_ts_node in self.graph['ts'].successors(f_ts_node): #后继节点
        ...
        # rospy.loginfo('乘积状态机有状态 %d 个，转移 %s 个' %(len(self.nodes()), len(self.edges())))
        sys.stdout.write('乘积状态机有状态 %d 个，转移 %s 个' %(len(self.nodes()), len(self.edges())))
...
class ProdAut_Run(object):
    ...
    def plan_output_finite(self, product):
        ...
        sys.stdout.write('LTL Planner: New local action plan: ' + str(self.action_sequence))
    ...
    def plan_output(self, product):
        ...
        sys.stdout.write('LTL Planner: Prefix plan: ' + str(self.pre_plan))
        sys.stdout.write('LTL Planner: Suffix plan: ' + str(self.suf_plan))