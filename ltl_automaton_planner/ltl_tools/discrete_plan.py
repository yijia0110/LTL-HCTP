...
def dijkstra_plan_networkX(product, gamma=10):
    ...
    if runs:
        ...
        print('Dijkstra_plan_networkX done within %.2fs: prefix cost %.2f, sufix cost %.2f' %(time.time()-start, precost, sufcost))
...
def dijkstra_plan_bounded(product, time_limit=3, gamma=10):
    ...
    # sys.stdout.writen('dijkstra plan started!')
    print('dijkstra plan started!')
...
def compute_path_from_pre(pre, target): # 输出任务规划的结果列表
    ...
    while n in pre:
        ...
        if not pn_list: # 如果内容都探索完了，就脱出循环
            break
        # print(pn_list)
        pn = pn_list[0] # 如果没有，那就进行路径赋值
    ...
    if changed_regs:
        for (index, prod_edge) in enumerate(run.pre_prod_edges):
            ...
            if t_ts_node not in succ_prod:
                    print('The current plan prefix contains invalid edges, need revision!')
        ...
        for (index, prod_edge) in enumerate(run.suf_prod_edges):
            ...
            for prod_node_to, weight in product.graph['ts'].fly_successors(f_ts_node):
                succ_prod.add(prod_node_to)
            if t_ts_node not in succ_prod:
                    print('The current plan suffix contains invalid edges, need revision!')
